...
class FileSystemHandler:
    ...
    def __init__(self, destinationDirectory):
        if not pth.exists(destinationDirectory):
            os.makedirs(destinationDirectory)
        if not pth.isdir(destinationDirectory):
            raise ValueError(f"Invalid directory : {destinationDirectory}")
    ...
    def classIndex(self, perspective):
        ...
        return self._join(
            prefix, "_index.html"
        )  # not that a class named index overwrites the class index
