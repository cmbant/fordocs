...
from .source_model import session, File, ProgramFile, Module, Class, \
    FileSubroutine
from .fshandler import FileSystemHandler
import os, re, treelib
...
class HTMLDocMaker(object):
    ...
    ARGUMENT_CLASS_EXTRACTOR_REGEX = re.compile("(class|type)\s*\((?P<class>\w+)\)",
                                                re.IGNORECASE)  # also match for things like 'integer'
    ...
    def _generateProgramDocs(self):
        ...
        for dbprogram in dbprograms:
            template_args = self._templateArgsForFile(
                dbprogram)  # a ProgramFile is the same as File. Difference in template
            if NOISY:
    ...
    def _generateModuleDocs(self):
        ...
        for dbmodule in dbmodules:
            ...
            module_file_doc = self._fshandler.fileDocForPath(module_file.name,
    ...
    def _generateClassDocs(self):
        ...
        for dbclass in dbclasses:
            ...
            trees = self._parseTrees([dbclass],
                                     perspective=FileSystemHandler.FROM_CLASS_FOLDER)  # remember, template requires arrays
    ...
    def _fullTreeForClass(self, dbClass, perspective, currentTree=None, originalId=False, parentIdentifier="root",
        ...
        if currentTree is None:
            ...
            self._createTreeNode(currentTree, dbClass, perspective, parentIdentifier,
        ...
        if not dbClass.name in split_class_names:
            ...
            if dbchildren:
                for dbchild in dbchildren:  # when there are no more children, it's over#
                    ...
                    _, node_classes = self._fullTreeForClass(dbchild, perspective, currentTree, False,
    ...
    def _treeForClasses(self, aClass, dbClasses, perspective, currentTree=None, parentIdentifier="root"):
        ...
        for child in children:
            _, included = self._treeForClasses(child, dbClasses, perspective, currentTree,
    ...
    def _parseDependencies(self, dbDependencies, perspective):
        ...
        for dbdep in dbDependencies:
            ...
            user_defined_dependency = user_defined_dependencies.filter(
                func.lower(Module.name) == func.lower(dbdep.name)).first()
    ...
    def _parseFullTrees(self, dbClasses, perspective, separate_top_classes=[]):
        ...
        for name in separate_top_classes:
            for cls in dbClasses:
                if cls.name == name:
                    class_tree, included_classes = self._downTreeForClass(cls, perspective,
                                                                          split_class_names=separate_top_classes)
                    already_included_classes.update(included_classes)
                    if len(included_classes) > 1:
                        trees.append(class_tree)
        for cls in dbClasses:
            if cls not in already_included_classes:
                class_tree, included_classes = self._fullTreeForClass(cls, perspective,
                                                                      split_class_names=separate_top_classes)
                already_included_classes.update(included_classes)
                if len(included_classes) > 1:
                    trees.append(class_tree)
