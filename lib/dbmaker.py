    ...
    def fillModel(self, sourceFolders=[], match='*.f90', file_list=[], excludes=[]):
        ...
        def excluded(fname):
            for ex in compiled_excludes:
                if ex.match(fname): return True
            return False

        if sourceFolders and isinstance(sourceFolders, six.string_types): sourceFolders = [sourceFolders]
        for sourceFolder in sourceFolders:
            ...
            for root, dirs, files in os.walk(sourceFolder, topdown=True):
                for dir in copy.copy(dirs):
                    if dir[0] == '.' or excluded(dir): dirs.remove(dir)
        ...
        for full_source_path in file_list:
            ...
            with open(full_source_path, 'r') as fhandle:
                source = fhandle.read()
